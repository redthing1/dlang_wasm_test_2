...
extern (C) void _d_assert(string file, uint line) @trusted @nogc pure {
    arsd.webassembly.eval(
        q{ console.error("Assert failure: " + $0 + ":" + $1); /*, "[" + $2 + ".." + $3 + "] <> " + $4);*/ }, file, line); //, lwr, upr, length);
    arsd.webassembly.abort();
}
...
Value[] values(T : Value[Key], Value, Key)(T aa) @property {
    ...
    static if (is(const(Value[Key]) == const(T)))
        ...
        const(Value[Key]) realAA = aa;
    auto res = () @trusted {
        auto a = cast(void[]) _aaValues(*cast(inout(AA)*)&realAA, Key.sizeof, Value.sizeof, typeid(
                Value[]));
        return *cast(Value[]*)&a;
    }();
...
private enum bool isSafeCopyable(T) = is(typeof(() @safe {
            union U {
                T x;
            }
...
void update(K, V, C, U)(ref V[K] aa, K key, scope C create, scope U update)
        if (is(typeof(create()) : V) && (is(typeof(update(aa[K.init])) : V) || is(
            typeof(update(aa[K.init])) == void))) {
    bool found;
...
version (inline_concat) extern (C) void[] _d_arraycatnTX(const TypeInfo ti, scope byte[][] arrs) @trusted {
    auto elemSize = ti.next.size;
    size_t length;
    foreach (b; arrs)
        length += b.length;
...
version (inline_concat) extern (C) byte[] _d_arraycatT(const TypeInfo ti, byte[] x, byte[] y) {
    import core.arsd.objectutils;
...
T[] dup(T)(scope T[] array) pure nothrow @trusted
        if (__traits(isPOD, T) && !is(const(T) : T)) {
    T[] result;
    foreach (ref e; array) {
        result ~= e;
    }
